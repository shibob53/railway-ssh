...
ENV Ngrok=${Ngrok}
RUN apt install ssh wget -y > /dev/null 2>&1
RUN apt install curl unzip sudo -y > /dev/null 2>&1
RUN apt clean
...
RUN mv ~/AdGuardHome/AdGuardHome /opt/AdGuardHome/
RUN chown -R root:root /opt/AdGuardHome
RUN chmod -R o-rwx /opt/AdGuardHome
RUN /opt/AdGuardHome/AdGuardHome -s install
RUN chmod 755 /1.sh
